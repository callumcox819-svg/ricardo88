...
def main() -> None:
    ...
    if webhook_base:
        if not webhook_base.startswith("https://"):
            raise RuntimeError(
                "WEBHOOK_BASE_URL должен начинаться с https://  (пример: https://xxxx.up.railway.app)"
            )
        ...
        # bootstrap_retries=-1 => бесконечные попытки подняться, даже если Telegram API временно недоступен.
